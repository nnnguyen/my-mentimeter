...
describe('WordCloudService', () => {
  ...
  let prisma: { wordAggregate: { findMany: jest.Mock }; response: { count: jest.Mock } };
  ...
  it('returns words sorted by count desc (delegated to the DB query), totalResponses, and uniqueWords', async () => {
    const words = [
